...
namespace ZAnGian
{
    public class GameObjectV3 : GameObject
    {
        ...
        public override MemWord? GetPropertyAddress(ushort targetPropId)
        {
            ...
            while (true)
            {
                MemWord currAddr = new MemWord(propAddr.Value); //a simple assignment works by reference
        ...
        public override void PutPropertyValue(ushort targetPropId, MemValue propValue)
        {
            ...
            if (propLen == 1)
            {
                ...
                _memory.WriteByte(propAddr+1, (byte)(propValue.FullValue & 0xff));
            }
            else if (propLen == 2)
            {
                _memory.WriteWord(propAddr+1, propValue.FullValue);
            }
            else
        ...
        public override MemWord? GetNextPropertyId(ushort startPropId)
        {
            ...
            else
            {
                ...
                propAddr += propLen + 1; //skip to next property
            }
